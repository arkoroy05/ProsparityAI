...
export const parseLeadsFromCSV = async (fileContent) => {
  try {
    ...
    return results.map(row => {
      ...
      const phoneFields = ['phone', 'phonenumber', 'phone_number', 'telephone', 'mobile', 'contact', 'tel', 'cell'];
      const phone = phoneFields.reduce((found, field) => found || normalizedRow[field], '');
...
export const validateCSVData = (parsedData) => {
  ...
  const hasPhone = normalizedKeys.some(key => 
    ['phone', 'phonenumber', 'phone_number', 'telephone', 'mobile', 'contact', 'tel', 'cell'].includes(key)
  );
