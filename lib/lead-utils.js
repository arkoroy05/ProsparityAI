...
export const formatPhoneNumber = (phoneNumber) => {
  ...
  if (digitsOnly.length >= 12 && digitsOnly.startsWith('91') && !hasPlus) {
    return `+${digitsOnly.substring(0, 2)} ${digitsOnly.substring(2)}`;
  }
  
  // Handle US/Canada numbers (1xxxxxxxxxx)
  if (digitsOnly.length === 11 && digitsOnly.startsWith('1') && !hasPlus) {
    return `+${digitsOnly.charAt(0)} (${digitsOnly.substring(1, 4)}) ${digitsOnly.substring(4, 7)}-${digitsOnly.substring(7)}`;
  }
