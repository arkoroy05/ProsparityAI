...
export class AICallAgent {
  ...
  async processSpeech(speechInput) {
    ...
    console.log('Processing speech input:', speechInput);

    if (!speechInput || speechInput.trim() === '') {
      // Provide varied responses for empty speech input
      const emptyInputResponses = [
        "I'm sorry, I didn't catch that. Could you please repeat?",
        "I didn't quite hear what you said. Could you please speak a bit louder?",
        "I apologize, but I couldn't understand that. Could you please try again?",
        "I'm having trouble hearing you. Could you please speak more clearly?",
        "Sorry about that, I missed what you said. Could you repeat that?"
      ];

      // Select a random response for variety
      const randomIndex = Math.floor(Math.random() * emptyInputResponses.length);
      this.debug('Empty speech input detected, using fallback response');
      return emptyInputResponses[randomIndex];
    }
  ...
  async generateContextualResponse(userInput, context) {
    try {
      ...
      if (!userInput || userInput.trim() === '') {
        throw new Error('Empty user input provided to generateContextualResponse');
      }

      // Ensure model is initialized
      if (!this.model) {
        this.debug('Model not initialized, attempting to initialize now');
        try {
          await this.initialize();
        } catch (initError) {
          this.debug('Failed to initialize AI model on demand:', initError);
          throw new Error('AI model initialization failed: ' + initError.message);
        }
      }
      ...
      let aiPersona = '';
      if (context && context.companyInfo?.ai_instructions) {
        this.debug('Using company-specific AI instructions');
        aiPersona = context.companyInfo.ai_instructions;
      } else if (context && context.aiSettings?.persona) {
  ...
  handleError(error) {
    ...
    if (this.retryCount >= 3) {
      ...
      const finalResponses = [
        "I apologize, but I'm having some technical difficulties with our connection. Let me have one of our team members call you back to continue this conversation. Thank you for your time.",
        "I'm experiencing some technical issues that are preventing me from continuing effectively. I'll arrange for one of our specialists to follow up with you directly. Thank you for your patience.",
        "It seems we're having some connection problems. I'll make sure a team member reaches out to you to continue this conversation properly. Thank you for your understanding."
      ];
    ...
    if (errorMessage.includes('model') || errorMessage.includes('ai') || errorMessage.includes('initialize')) {
      ...
      const modelResponses = [
        "I apologize for the brief pause. Could you please tell me more about your business needs so I can better assist you?",
        "Sorry about that momentary delay. To help you better, could you share more details about what you're looking for?",
        "Pardon the interruption. I'd like to understand your business requirements better - could you elaborate a bit more?"
      ];
      ...
      // Database issues
      const dbResponses = [
        "I'm having trouble accessing some information. While that's being resolved, could you tell me more about what you're looking for in a solution like ours?",
        "I'm temporarily unable to access some of our system data. In the meantime, could you share more about your current challenges?",
        "There seems to be a brief issue with our information system. While that's resolving, I'd love to hear more about your business goals."
      ];

      return dbResponses[Math.floor(Math.random() * dbResponses.length)];
    } else if (errorMessage.includes('validation') || errorMessage.includes('response')) {
      // Response generation issues
      const validationResponses = [
        "I want to make sure I understand correctly. Could you please explain your question or concern in a bit more detail?",
        "To ensure I'm addressing your needs properly, could you elaborate a bit more on what you just mentioned?",
        "I'd like to better understand your perspective. Could you provide a bit more context to your question?"
      ];
      ...
      const genericResponses = [
        "I apologize for the confusion. To better assist you, could you please rephrase your question or share more about your business needs?",
        "I want to make sure I'm providing the most helpful information. Could you share a bit more about what you're looking for?",
        "I'd like to get back on track. Could you tell me more specifically what you're interested in learning about our services?",
        "Let me make sure I understand your needs correctly. Could you provide a bit more detail about your question?"
      ];
