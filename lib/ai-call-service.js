...
export async function executeCall(callId) {
  try {
    ...
    // Prepare Twilio callback URL with parameters
    const params = [
      ['callId', callId],
      ['taskId', callData.task_id],
      ['leadId', callData.lead_id],
      ['companyId', callData.company_id]
    ];
    ...
    const callbackUrl = `${process.env.NEXT_PUBLIC_APP_URL}/api/calls/webhook?${queryString}`;
    ...
    // Store the Twilio call SID and talking points
    await supabase
      .from('scheduled_calls')
      .update({
...
// Helper function to make the actual Twilio call
async function makeCall(to, from, callbackUrl) {
  try {
    if (!to || !from || !callbackUrl) {
      throw new Error('Missing required parameters for making call');
    }

    console.log('Initiating Twilio call:', { to, from, callbackUrl });
    ...
    const call = await twilioClient.calls.create({
      ...
      record: true,
      recordingStatusCallback: `${process.env.NEXT_PUBLIC_APP_URL}/api/twilio/recording-callback`
    });
...
// Helper function to generate talking points for the AI agent
async function generateTalkingPoints(leadInfo, companyInfo) {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });
    
    const prompt = `
    Generate key talking points for a sales call with the following context:
    ...
    - Previous Interactions: ${leadInfo.previous_interactions || 'None'}

    Our Company Information:
    - Name: ${companyInfo.name || 'Prosparity'}
    - Product: AI-powered sales solution
    - Key Benefits: Automation, improved conversion rates, data-driven insights

    Generate 3-5 key talking points that:
    1. Address the lead's specific industry/role
    2. Connect our solution to their potential needs
    3. Include relevant questions to ask
    4. Highlight key differentiators
    5. Address common objections proactively

    Format the response as a JSON array of talking points.`;
    ...
    return [
      "Discuss how our AI solution can improve sales efficiency",
      "Explore their current sales process and pain points",
