  ...
  try {
    ...
    switch (task.task_type?.toLowerCase()) {
      ...
      case 'message':
        ...
        break;
      default:
        console.warn(`Task type ${task.task_type} not implemented yet`);
        throw new Error(`Task type ${task.task_type} not supported`);
    }
  ...
  try {
    ...
    const callResult = await makeCall(
      ...
      callbackUrl,
      {
        taskId: task.id,
        leadId: lead.id,
        aiInstructions: task.ai_instructions || ''
      }
    );
    
    console.log('Call initiation result:', {
      success: !!callResult.callSid,
    ...
    if (callResult.error) {
      throw new Error(`Twilio error: ${callResult.error}`);
    }
